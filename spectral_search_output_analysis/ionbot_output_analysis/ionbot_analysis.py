...
def main(args):
    ...
    ibdf_ontonly_original=file_import.concatenate_csvs(args['ont'])
    ibdf_refonly_original=file_import.concatenate_csvs(args['ref'])
    ibdf_combi_original=file_import.concatenate_csvs(args['cvf'])
    ibdf_combi_pg_original=file_import.concatenate_csvs(args['cvc'])

    #inital QC
    print("plotting initial QC")
    plots.plot_scores(ibdf_ontonly_original.dropna(),ibdf_refonly_original.dropna(),ibdf_combi_original.dropna())
    plots.plot_scores_pg(ibdf_combi_original.dropna(),ibdf_combi_pg_original.dropna())
    plots.plot_target_decoy(ibdf_combi_original.dropna(),"qc_pearsonr_decoy_varfree.png", plot_title="Search result variant-free")
    plots.plot_target_decoy(ibdf_combi_pg_original.dropna(),"qc_pearsonr_decoy_varcont.png", plot_title="Search result variant-containing")
    ...
    main_functions.discrepancy_check(var_vc,var_vf, ibdf_combi, ibdf_combi_pg)
    ...
    plots.plot_final_venns(var_vc,var_vf,mut_cpdt,mutprotset)
    return("Finished")
