...
def main_replacement(percolatordir,csvname,ibcsvpath):
    ...
    percolator_df.columns=["scan_id","proteinIds"]
...
def read_df_in_chunks(directory,percolator_subset,csvname, chunksize):
    ...
    for chunk in df_chunk:  
        ...
        chunk_processed = pd.merge(chunk_filter,percolator_subset, how='left', on='scan_id')
...
def file_list(path):
    ...
    for csvfile in os.listdir(directory):
        ...
        if csvname.endswith('ionbot.csv'):
...
def main(ibcsvpath,percolatordir,outfile):
    ...
    results=[pool.apply_async(main_replacement,args=(percolatordir,x,ibcsvpath)) for x in tqdm(list_csvs)]
